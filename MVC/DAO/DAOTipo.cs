...
namespace R5SALAO.MVC.DAO
{
    class DAOTipo
    {
        ...
        public void Save(ModelTipo model)
        {
            try
            {
                conexao.Conectar();
                sql = "INSERT INTO TBTIPOCATEGORIA(nome)VALUES(@nome)";
        ...
        public void Update(ModelTipo model)
        {
            try
            {
                conexao.Conectar();
                sql = "UPDATE TBTIPOCATEGORIA SET nome=@nome where id=@id";
                comando = new SQLiteCommand(sql, conexao.con);
        ...
        public void Delete(int id)
        {
            try
            {
                sql = "DELETE FROM TBTIPOCATEGORIA WHERE id=" + id + "";
                conexao.Conectar();
                comando = new SQLiteCommand(sql, conexao.con);
                comando.ExecuteNonQuery();
                util.MessageDelete();
